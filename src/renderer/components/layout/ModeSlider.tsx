...
export default function ModeSlider() {
  ...
  return (
    ...
    >
      ...
      {/* Labels */}
      <span 
        className={`absolute left-2 text-[10px] font-medium transition-opacity duration-200 ${
          appMode === 'workbench' ? 'opacity-100' : 'opacity-0'
        }`}
      ...
      <span 
        className={`absolute right-2 text-[10px] font-medium transition-opacity duration-200 ${
          appMode === 'ready' ? 'opacity-100' : 'opacity-0'
        }`}
